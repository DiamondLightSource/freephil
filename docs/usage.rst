...
syntax. Major Phil features are:
  ...
  - The concepts of `base files` and `user files`. The syntax
...
Base files are written by the software developer and include
...
will process its base file and show only the most
...
arguments as substrings to the parameter names in the base files::
...
to a unique match in the base file. Otherwise Phil produces a helpful
...
``base_phil`` defines all available parameters including the
...
``base_phil.fetch()``. ``working_phil.show()`` produces::
...
fetch_diff: difference between base_phil and working_phil
-----------------------------------------------------------

The ``.fetch()`` method introduced above produces a complete copy
of the Phil base with all user definitions and scopes merged in.
If the Phil base is large, the output of ``working_phil.show()`` will
...
To get just the difference between the base and the working
...
We need to bring in ``base_phil`` again because all the meta
...
is a multiple definition in a base file::
...
Without the ``.multiple = True`` in the base file, ``.fetch()``
retains only the *last* definition found in the base and all user
...
would be a plausible base file::
...
Another option for building base files from a library of building
...
script). The ``base_phil`` object in the imported module must be
a pre-parsed Phil scope or a plain Phil string. The content of the
``base_phil`` scope is inserted into the scope of the ``include
scope`` statement.

``include`` directives enable hierarchical building of
base files without the need to copy-and-paste large fragments
explicitly. Duplication appears only in automatically generated user
files. I.e. the programmer is well served because a system of base
...
that we introduced earlier to merge user files given a base file.
...
Base files generally shouldn't make use of the semicolon syntax, even
...
corresponding base is commented out.
...
base files without any loss of information. ``attributes_level=3``
...
To catch spelling errors, or to alert users to changes in the base
