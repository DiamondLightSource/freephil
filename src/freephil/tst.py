...
from libtbx.test_utils import Exception_expected, show_diff, anchored_block_show_diff
...
def exercise_string_quote_and_tokenize():
    ...
    def tvl(input_string):
        return phil.tokenize_value_literal(input_string=input_string, source_info=None)
    ...
    words = tvl(
        input_string=phil.tokenizer.quote_python_str(quote_token='"', string=value)
    )
    assert len(words) == 1
...
class converter_implementation(int_phil_converters):
    def __str__(self):
        if self.factor == 1:
            return "libtbx.phil.tst.converter_factory"
...
"""
        ...
        assert (
            str(e) == ".type=libtbx.phil.tst.none: object"
...
"""
        ...
        assert (
            str(e) == '"libtbx.phil.tst.improper_phil_converters" is not'
...
"""
        ...
        assert (
            str(e) == 'Error evaluating definition type "libtbx.phil.tst.int'
...
def test_exception(input_string, exception_string=None):
    try:
        phil.parse(input_string=input_string)
...
def exercise_syntax_errors():
    test_exception("'a'", """Unquoted word expected, found 'a' (input line 1)""")
    test_exception("a", "Unexpected end of input.")
    ...
    test_exception("s{y=1{", 'Syntax error: unexpected "{" (input line 1)')
    test_exception("s{y=#", "Missing value for y (input line 1)")
    ...
    test_exception(";{}", 'Syntax error: unexpected ";" (input line 1)')
    test_exception("s{;}", 'Syntax error: unexpected ";" (input line 1)')
    test_exception(
        "a=b\n.foo none", "Unexpected definition attribute: .foo (input line 2)"
    )
    test_exception('a=b\nc "abc', "Syntax error: missing closing quote (input line 2)")
    test_exception("1 {", 'Syntax error: improper scope name "1" (input line 1)')
    test_exception("scope\n.foo", "Unexpected scope attribute: .foo (input line 2)")
    test_exception("a. 2", 'Syntax error: improper definition name "a." (input line 1)')
    test_exception("a.include=None", 'Reserved identifier: "include" (input line 1)')
    test_exception("include {}", 'Reserved identifier: "include" (input line 1)')
    test_exception("a.include.b.c {}", 'Reserved identifier: "include" (input line 1)')
...
def exercise_phil_on_off_end():
    ...
    assert (
        phil.parse(input_string="#phil __OFF__\n#phil __ON__ \na=1").as_str()
        == "a = 1\n"
    )
    assert phil.parse(input_string="#phil __END__\n#phil __ON__ \na=1").as_str() == ""
    params = phil.parse(
...
""",
            ...
            assert str(e) == "include scope: no module libtbx.phil.t_s_t (input line 1)"
        else:
            raise Exception_expected
        try:
            phil.parse(
...
"""
    ...
    master_optional_multiple = phil.parse(
        input_string="""\
s
  .multiple=1
  .optional=yes
...
""",
    ...
    #
    pcl = phil.process_command_line(args=["bar.max=4943"], master_string=master_string)
...
""",
    ...
    pcl = phil.process_command_line(args=["892c8632"], master_string=master_string)
...
"""
        ...
        assert (
            str(e) == 'scope "s" .call: "libtbx.phil.tst.scope_call_not_callable"'
...
"""
        ...
        assert str(e).startswith('scope "s" .call=libtbx.phil.tst.scope_call_func(: ')
        assert str(e).endswith(" (input line 2)")
    else:
        raise Exception_expected
    #
...
"""
        ...
        assert (
            str(e) == 'scope "s" .call=libtbx.phil.tst.scope_call_func(a=b=c):'
            " SyntaxError: invalid syntax (line 1) (input line 2)"
        )
    else:
        raise Exception_expected
...
""",
    ...
    try:
        ...
        assert (
            str(e) == 'scope "u" .call=libtbx.phil.tst.scope_call_func(a=1, b=2)'
