    ...
    while True:
        ...
        if (
            ...
            if word.value != "#":
                ...
                break
            have_comment = True
        elif word.quote_token is not None or (
            last_word.quote_token is None and last_word.value == "\\"
        ):
            if not have_comment:
                result.append(word)
...
def collect_objects(
    ...
    while True:
        ...
        if lead_word is None:
            break
        if lead_word.value == "#phil" and lead_word.line_number != prev_line_number:
            word = word_iterator.pop_unquoted()
            if word.value == "__END__":
                break
            if word.value == "__ON__":
                continue
            ...
            if (
                word_iterator.scan_for_start(
                    intro=lead_word.value, followups=["__END__", "__ON__"]
                )
                == 0
            ):
                break
        ...
        if stop_token is not None and lead_word.value == stop_token:
            return
        if lead_word.value == "{":
            raise RuntimeError('Syntax error: unexpected "{"%s' % lead_word.where_str())
        ...
        if word.quote_token is None and (
            ...
            while True:
                ...
                if word.value[:1] == "!":
                    ...
                    is_disabled = False
                if word.value[:1] != "." or not scope.has_attribute_with_name(
                    word.value[1:]
                ):
                    raise RuntimeError(
            ...
            if lead_word.value[:1] != ".":
                ...
                if (
                    active_definition is None
                    or not active_definition.has_attribute_with_name(
                        lead_word.value[1:]
                    )
                ):
                    raise RuntimeError(
                        "Unexpected definition attribute: %s%s"
    ...
    if stop_token is not None:
        ...
        raise RuntimeError(
            'Syntax error: no matching "%s" for "%s" at %s'
