...
refinement.ncs.restraint_group {
  ...
  selection = "chain D and resseq 1:100"
    ...
    names = i.search_phil_text("Restraint group", match_all=True, labels_only=True)
    assert len(names) == 0
    names = i.search_phil_text("Restraint group", match_all=True, labels_only=False)
    assert len(names) == 3
...
refinement.ncs.restraint_group {
  ...
  selection = "chain D"
    ...
    params = i.get_python_object()
    assert (
        params.refinement.secondary_structure.helix[0].selection
        == "chain A and resseq 10:20"
    )
    t3 = time()
    i.merge_phil(phil_string=phil_str_2, only_scope="refinement.secondary_structure")
    t4 = time()
    params = i.get_python_object()
    assert (
        params.refinement.secondary_structure.helix[0].selection
    ...
    i.merge_phil(phil_string=phil_str_3, only_scope="refinement.ncs.restraint_group")
    params = i.get_python_object()
    assert params.refinement.ncs.restraint_group[0].reference == "chain A"
    i.merge_phil(phil_string=phil_str_4, only_scope="refinement.ncs.restraint_group")
    params = i.get_python_object()
...
"""
    ...
    files_in = [
        (
            "/Users/nat/Documents/beta-blip/beta_blip_P3221.mtz",
            "Data file",
            "phaser.hklin",
        ),
        (
            "/Users/nat/Documents/beta-blip/blip.seq",
            "Sequence file",
            "phaser.composition.chain.sequence_file",
        ),
        (
            "/Users/nat/Documents/beta-blip/blip.pdb",
            "Ensemble model",
...
phaser.search {
  ...
  copies = 1
    ...
    i.merge_phil(phil_file="phaser.eff")
    p = i.get_python_object().phaser
    assert p.hklin == "/Users/nat/Documents/projects/beta-blip/beta_blip_P3221.mtz"
