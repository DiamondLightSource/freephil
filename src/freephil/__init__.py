...
standard_identifier_continuation_characters = dict(standard_identifier_start_characters)
for c in ".0123456789":
...
def int_from_words(words, path):
    ...
    if result is not None and result is not Auto:
        if isinstance(result, float) and round(result) == result:
            result = int(result)
...
class floats_converters(object):
    ...
    def __init__(
        self, size=None, size_min=None, size_max=None, value_min=None, value_max=None
    ):
...
class definition(object):
    ...
    def fetch(self, source, diff=False):
        if diff:
            return self.fetch_diff(source=source)
    ...
    def get_without_substitution(self, path):
        if self.is_disabled or self.name != path:
            return []
        return [self]
    ...
    def extract(self, parent=None):
        ...
        try:
            type_from_words = self.type.from_words
...
class scope_extract(object):
    ...
    def __phil_path__(self):
        ...
        return ".".join([self.__phil_parent__.__phil_path__(), self.__phil_name__])
...
class scope(object):
    ...
    def fetch(
        self, source=None, sources=None, track_unused_definitions=False, diff=False
    ):
        combined_objects = []
    ...
    def fetch_diff(self, source=None, sources=None, track_unused_definitions=False):
        return self.fetch(
            source=source,
            sources=sources,
